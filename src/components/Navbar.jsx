...
export default function Navbarinuse() {
    ...
    return (
        <>
                <div className="fixed bottom-0 left-0 w-full h-16 bg-white rounded-t-3xl border-t border-gray-300 z-50">
                    <div className="relative mt-3">
                    <div className="absolute top-[-38px] left-[50%] transform -translate-x-1/2 z-20">
                    <button 
                        className="bg-blue-600 p-3 rounded-3xl hover:scale-110 duration-300 transition-transform"
                        onClick={handleToggle}
                    >
                        <Plus className={`text-white transition-transform duration-300 ${isActive ? "rotate-45" : "rotate-0"}`} />
                    </button>
                        {isActive && (
                            <div className="absolute w-40 bg-blue-600 rounded-lg bottom-13 left-6 flex flex-col items-center justify-center gap-2 p-2 shadow-2xl animate-in fade-in slide-in-from-bottom duration-300">
